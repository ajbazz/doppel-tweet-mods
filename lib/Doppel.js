...
class Doppel {
	...
	updateLex(){
		...
		var tweetList = [];
		this.getTweets(this.source[0].userName, this.source[0].mostRecentTweetId, 0, 0, tweetList, this);
	}

	// Receives a single set of Tweets and will rerun if more are available.
	getTweets(userName, afterId, beforeId, sourceNum, list, doppel){
		// Create API URL
		...
		this.link.get(url, function(error, tweets, response){
			...
			for(var i = 0; i < totalTweets; i++){
				...
				if(id > doppel.source[sourceNum].mostRecentTweetId){
					doppel.source[sourceNum].mostRecentTweetId = id;
				}
			...
			console.log('New max: ' + doppel.source[0].mostRecentTweetId);
		});
	}
}
